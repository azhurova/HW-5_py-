...
# Путь к файлу должен передаваться в аргументах декоратора. Функция test_2 в
...
def logger(path):
    def __logger(old_function):
        def new_function(*args, **kwargs):
            ...
            with open(, 'ab') as log_file:
                log_file.write((','.join([datetime.now().strftime("%m/%d/%Y, %H:%M:%S"),
                                         f'''Вызов {old_function.__name__}, с аргументами {args} и {kwargs}''',
                                         f'Результат: {result}']) + '\n').encode('utf-8'))
                log_file.close()
            return result
